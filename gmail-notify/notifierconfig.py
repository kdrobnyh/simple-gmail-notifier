...
class GmailConfigWindow:
    ...
    def __init__(self):
        ...
        if (self.readLoginFromFile is None and (self.options["gmailusername"] is not None and self.options["gmailpassword"] is not None)):
    ...
    def readConfig(self):
        ...
        readFiles = self.config.read(os.path.expanduser("~/.notifier.conf"))
        if (len(readFiles) == 1):
    ...
    def onOkay(self, widget, callback_data=None):
        ...
        #if (self.savePassword.get_active()):
        # try:
        #  tempLogin = notifieratom.GmailAtom(self.options["gmailusername"], self.options["gmailpassword"]))
        #  tempLogin.refreshInfo()
        # except:
        #  print "Unexpected error: ", sys.exc_info()[0]
        #  errorString = errorString + "Login appears to be invalid\n"

        if True:  # (len(errorString) == 0):
            ...
            if (not self.savePassword.get_active()):
                ...
                if (self.options["gmailusername"] is not None and self.options["gmailusername"] != ""):
                    self.options["gmailusername"] = bz2.compress(self.options["gmailusername"]).encode('base64')
                if (self.options["gmailpassword"] is not None and self.options["gmailpassword"] != ""):
                    self.options["gmailpassword"] = bz2.compress(self.options["gmailpassword"]).encode('base64')
